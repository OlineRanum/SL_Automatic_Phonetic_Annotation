...
from PoseTools.handedness.utils import calculate_center_of_mass, calculate_velocity, get_masked_arr, get_normalized_coord, extract_names_from_filtered_file
from PoseTools.handedness.graphics import plot_position, plot_velocity, plot_integrated_velocities
...
def process_direcotory(directory_path, output_file, filtered_file_path):
    ...
    pose_files = [f for f in os.listdir(directory_path) if f.endswith('.pose')]
    ...
    with open("PoseTools/results/handedness.txt", "w") as handedness_file:
        ...
        for pose_file in tqdm(filtered_pose_files, desc="Processing pose files"):
