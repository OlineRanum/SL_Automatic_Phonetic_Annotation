...
from PoseTools.src.models.graphTransformer.data.dataloader import GraphDataReader, GraphDataLoader
import argparse
import os
import torch
...
def main(args):
    ...
    model = HandshapeGAT(in_channels=3, hidden_channels=128, out_channels=20, heads=4)
    ...
    #main_scheduler =  CyclicLR(optimizer, base_lr=1e-4, max_lr=1e-3, step_size_up=5, mode='triangular2')
    ...
    save_dir = os.path.abspath("/home/gomer/oline/PoseTools/src/models/graphTransformer/trained_models")  # Directory to save best model
    train_model(model, train_loader, val_loader, optimizer, criterion, device, args.epochs, save_dir, warmup_scheduler=warmup_scheduler, main_scheduler=None, test_loader=test_loader, eval = True)
...
if __name__ == "__main__":
    ...
    path = "/home/gomer/oline/PoseTools/data/metadata/output/20c/20c_2000.json"
    print(path)
