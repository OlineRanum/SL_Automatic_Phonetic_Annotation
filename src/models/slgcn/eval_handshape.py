...
def main():
    ...
    parser.add_argument(
        '--config', 
        type=str, 
        default='test_hamer.yml',  # Default to handedness.yaml configuration file
        help="Path to configuration file (default: test_hamer.yml)"
    )
    parser.add_argument(
        '--input_path', 
        type=str, 
    ...
    #get classes from /home/gomer/oline/PoseTools/data/metadata/output/sh/value_to_id.txt
    #predicted class will output number, it will be binded to class name from txt file. for example class 0 equals rule 0 from txt file
    #split the txt file by \n and then split by space, then get the second element from the split
    txtfile = open("/home/gomer/oline/PoseTools/data/metadata/output/sh/value_to_id.txt", "r")
    classes = txtfile.read().split("\n")
    txtfile.close()
