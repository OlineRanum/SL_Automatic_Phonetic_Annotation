...
class HandshapeGAT(nn.Module):
    ...
    def forward(self, data):
        ...
        return out  # Return both final output and embeddings

def augment_data(pose, device):
    """
    Adds Gaussian noise to the pose data as a form of data augmentation.
    """
    noise = torch.normal(mean=0, std=0.01, size=pose.shape).to(device)
    return pose + noise
...
def train_model(model, train_loader, val_loader, optimizer, criterion, device, epochs, save_dir, warmup_scheduler, main_scheduler,test_loader = None, eval = True):
    ...
    """
    wandb.init(project="handshape_classification", config={"epochs": epochs, "batch_size": train_loader.batch_size})
    model.to(device)
    ...
    for epoch in range(epochs):
        ...
        for batch in train_progress_bar:
            ...
            # Apply data augmentation (only to pose data, i.e., `batch.x`)
            #augmented_batch = batch.clone()  # Clone the batch to avoid modifying the original data
            #if random.random() < 0.5:  # 50% chance to augment each instance
            #    augmented_batch.x = augment_data(augmented_batch.x, device)
            # Forward pass
            output = model(batch)  # Pass the augmented batch to the model
        ...
        if epoch < 5:
            ...
            main_scheduler.step(val_loss)  # Step the main scheduler after warmup
...
def evaluate_model(model, loader, criterion, device, save_predictions=True, pred_file_path=None):
    """
    ...
    with torch.no_grad():  # Disable gradient computation for evaluation
        ...
        for batch in progress_bar:
            ...
            # Update the progress bar description with the current average loss and accuracy
            progress_bar.set_postfix(loss=total_loss / (progress_bar.n + 1), accuracy=100 * correct / total)
...
def test_model(model, test_loader, criterion, device, best_model_path):
    ...
    pred_file_path = '/home/gomer/oline/PoseTools/results/logs/predictions_and_label.txt'
