...
class HandshapeGAT(nn.Module):
    ...
    def forward(self, data):
        ...
        x = self.fc1(x)
        embeddings = self.activation(x)  # Store the embeddings here
        out = self.fc2(embeddings)  # Final classification output

        return out, embeddings  # Return both final output and embeddings
...
def test_model(model, test_loader, criterion, device, model_path):
    ...
    with torch.no_grad():
        for data in test_loader:
            ...
            # Forward pass
            outputs, embeddings = model(data)  # Modify model to return embeddings too
            loss = criterion(outputs, labels)

            # Append embeddings and labels for t-SNE
